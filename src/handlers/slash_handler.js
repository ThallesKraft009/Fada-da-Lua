...
description_localizations: slashCommand.description_localizations,
    ...
    /*wait rest.put(Routes.applicationCommands(CLIENT_ID), { body: [] }).then(() => console.log(chalk.blue('Carregandos slashCommands....')))*/
...
let jsonData = fs.readFileSync('src/Json/commands.json');

let dados = JSON.parse(jsonData);

          dados = lista_cmd;

let novoJsonData = JSON.stringify(dados);

fs.writeFileSync('src/Json/commands.json', novoJsonData);
